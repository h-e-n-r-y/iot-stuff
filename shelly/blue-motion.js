// v0.1
// adapted from Matthias Kleine https://haus-automatisierung.com/hardware/shelly/2023/09/02/shelly-blu-motion.html
...
let CONFIG = {
    ...
    onMotionChange: function(mac, motion) {
        let bulb;
        let transition;
        switch(mac) {
            case "b0:c7:de:7c:91:7f":
                // Keller Mitte
                bulb = "192.168.1.197";
                break;
            case "b0:c7:de:3c:d5:dc":
                // Waschkeller
                bulb = "192.168.1.58";
                break;
            case "0c:ae:5f:59:c3:c5":
                // Werkzeugkeller
                bulb = "192.168.1.61";
                break;
            default:
        ...
        Shelly.call(
            "HTTP.GET",
            {url: "http://" + bulb + "/light/0?turn=" + motion + "&transition=" + transition});
        console.log("Motion(" + mac + ")", motion);
    },
};
/******************* STOP CHANGE HERE *******************/
...
function bleScanCallback(event, result) {
    ...
    if (typeof unpackedData.motion !== "undefined") {
        CONFIG.onMotionChange(unpackedData.address, unpackedData.motion === 1);
    }
}
